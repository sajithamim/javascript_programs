...
const user = users?.find((item) => item?.firstName === 'Madison' && item?.lastName === 'Marshall')
console.log("What is Madison Marshall's user id?", user?.id)

//  2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)
let first_cmnt = comments?.find((item) => item?.userId)
console.log("Who wrote the first comment (assuming the first comment is in position 0 of the comments array)", users.find(item => item?.id === first_cmnt?.userId))

// 3) Which user commented 'OK great thanks'?
let ok_user = comments.find((item) => item?.text === 'OK great thanks')
console.log("Which user commented 'OK great thanks'?", users.find((user) => user?.id === ok_user?.userId))

// 4) Add the user's first and last name to each comment in the comments array
const appendNameTocomment = comments.map(comment => {
    const {firstName , lastName} = users.find(user => user.id === comment.userId)
    return {...comment, firstName, lastName}
})
console.log("Modified Commnts array with first and last name --> ", appendNameTocomment);

// 5) Get a list of the users who haven't commented

let haventComment = users.filter(user => !comments.find(comment => comment.userId === user.id))
console.log("Get a list of the users who haven't commented ", haventComment);
